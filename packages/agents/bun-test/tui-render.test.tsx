...
test("starts on the agent dashboard with the transcript visible", async () => {
...
test("enter on agents switches to input; esc steps back through transcript", async () => {
  const store = makeStore();
  const setup = await testRender(<App store={store} send={() => {}} cancel={() => {}} />, {
    width: 100,
    height: 30,
...
test("x kills the selected running agent on agents and transcript", async () => {
  const store = makeStore();
  let killed = 0;
...
test("escape steps back: input → transcript → agents, agents is a no-op", async () => {
  const store = makeStore();
  const setup = await testRender(<App store={store} send={() => {}} cancel={() => {}} />, {
    width: 100,
    height: 30,
  ...
  try {
    await setup.renderOnce();
    // Start on agents (default). Esc on agents does nothing.
    await act(async () => setup.mockInput.pressEscape());
    await new Promise((resolve) => setTimeout(resolve, 50));
    expect(store.getSnapshot().focus).toBe("agents");
...
test("selects an agent and focuses the transcript with the mouse", async () => {
  const store = makeStore();
  const setup = await testRender(<App store={store} send={() => {}} cancel={() => {}} />, {
    width: 100,
    height: 30,
...
test("j/k and arrows navigate agents in A mode", async () => {
  const store = makeStore();
  store.apply({
    type: "agent.created",
    agent: {
