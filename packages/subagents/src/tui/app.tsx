...
export function App({
  ...
  useKeyboard((key) => {
    ...
    if (escape) {
      ...
      // On the subagent list: escape does nothing.
      return;
    }
    ...
    return (
      <box style={{ flexGrow: 1, alignItems: "center", justifyContent: "center" }}>
        <box style={{ flexDirection: "column", padding: 2 }}>
          <text fg={colors.muted}>No subagents yet.</text>
          <text fg={colors.dim} style={{ marginTop: 1 }}>
            Subagents appear here when pi starts one.
          </text>
        </box>
      </box>
    );
...
function AgentHeader({ agent }: { readonly agent?: AgentRecord }) {
  if (!agent) {
    return (
      ...
      >
        <text fg={colors.muted}>Select a subagent to inspect its transcript.</text>
      </box>
    );
  }
  const meta = statusMeta[agent.status];
...
function AgentTranscript({
  ...
  if (!agent) return <text fg={colors.muted}>Select a subagent to inspect its transcript.</text>;
