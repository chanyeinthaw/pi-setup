...
export type SubagentsToolInput =
...
export const executeSubagentsTool = (
  client: AgentClient,
  input: SubagentsToolInput,
  context: ToolContext,
) =>
  Effect.gen(function* () {
    ...
    else if (execution.capability === "find") {
...
function format(capability: string, result: any) {
  ...
  if (capability === "send") return `Sent message to "${result.agent.slug}".`;
  if (capability === "stop") return `Stopped "${result.agent.slug}" (${result.agent.status}).`;
