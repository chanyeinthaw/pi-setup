...
export const serviceName = "subagents.service";
...
export function renderSystemdUserService(options: {
  ...
  return `[Unit]
Description=Native Pi background subagents daemon
...
export const install = (options: { readonly executable: string; readonly path?: string }) =>
  Effect.gen(function* () {
    ...
    return target;
  }).pipe(mapPlatformError("Failed to install subagents user service."));
...
export const uninstall = Effect.gen(function* () {
  ...
  yield* systemctl("daemon-reload");
}).pipe(mapPlatformError("Failed to uninstall subagents user service."));
