...
export type CapabilityName = "start" | "find" | "status" | "send" | "stop" | "wait";
...
const capabilities: ReadonlyArray<Capability> = [
  {
    name: "start",
    description: "Start a persistent native Pi subagent in the background.",
  ...
  {
    name: "find",
    description: "Find subagents by parent session, project, name, or status.",
    keywords: ["find", "list", "search", "subagents"],
    required: [],
    input: {
  ...
  {
    name: "status",
    description: "Inspect a subagent's status and recent transcript activity.",
  ...
  {
    name: "send",
    description: "Steer a running subagent or continue a settled subagent.",
  ...
  },
  {
    name: "wait",
    description:
      "Wait for one subagent to settle and retrieve its result immediately if already finished.",
...
export function parseExecution(
  ...
  if (!capability) throw new Error(`Unknown subagent capability "${capabilityName}".`);
