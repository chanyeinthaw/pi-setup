...
| --------------- | ------------------------------- | --------------- | ------------------- | ---------------- |
| MySQL 8.4       | `mysql.database.internal:3306`  | `developer`     | `dev-mysql`         | `development`    |
| PostgreSQL 18.4 | `pg.database.internal:5432`     | `developer`     | `dev-postgres`      | `development`    |
| Redis 8         | `redis.cache.internal:6379`     | —               | `dev-redis`         | —                |
| MinIO S3 API    | `https://minio-s3.app.internal` | `minioadmin`    | `minioadmin`        | —                |
| MinIO console   | `https://minio.app.internal`    | `minioadmin`    | `minioadmin`        | —                |
...
```text
mysql://developer:dev-mysql@mysql.database.internal:3306/development
postgresql://developer:dev-postgres@pg.database.internal:5432/development
redis://:dev-redis@redis.cache.internal:6379/0
```

For S3 clients and SDKs, always use `https://minio-s3.app.internal` as the endpoint. Use `https://minio.app.internal` only for the browser management console. The username and password above act as the S3 access key and secret key. Do not configure development clients to use a localhost MinIO endpoint because clients may run on other Tailscale-connected development machines.
...
Friendly database/cache names and `.app.internal` endpoints are provided through the local/tailnet infrastructure. Test Tailscale Services with the actual protocol client rather than `tailscale ping`.
